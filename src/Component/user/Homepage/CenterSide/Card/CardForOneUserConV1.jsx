...
import { API, BASE_URL1 } from "../../../../../utils/Utils";
import { ConnectionFollowUser } from "../../../../../Redux/service/Follower";
import { DropShadow } from "../../../../../utils/Style";

export default function CardForOneUserConV1({ item }) {

  return (
    
    <div className={`${DropShadow}`}>
      {/* <div className="lg:w-[100%] md:w-[100%] xs:w-[90%] grid grid-rows-2 grid-flow-col gap-4"> */}
      <div className="w-full grid h-full content-between items-end p-6 rounded-[20px] line-clamp-1 break-all">
        <div className="w-full">
          <div className="flex">
            ...
            <div className="mx-5">
              <p className=" text-gray-500 font-bold font-SecondaryFont text-[20px]">
                {item.username}
              </p>
              <div className="mx-1 ">
                <div className=" ">
                  <p className="text-sm text-left text-green_custom font-[600] rounded-[26px] font-SecondaryFont">
                    {item.jobType}
                  </p>
                </div>
                <div className="my-1">
          ...
          <div className="mt-2">
            <div className="flex border-b-2 border-b-green_custom py-1">
              ...
              <p className="font-semibold line-clamp-3  text-green_custom text-[20px] px-2">
                Skills
              </p>
            </div>
            <div className="flex place-items-center ml-10 mt-2">
              {}
              <ul className="leading-2 line-clamp-3 list-none text-[16px] font-SecondaryFont">
                {item.skill &&
          ...
          <div className="mt-2">
            ...
            <div className="ml-10">
              <ul className="leading-5 w-full  line-clamp-3 list-none font-semibold pt-2">
                {item.experience &&
                  Object.entries(item.experience).map(([key, value]) => (
                    <li key={key} className="py-1 font-medium text-[16px]">
                      {key}
        ...
        <button className="bg-green_custom px-4 rounded-3xl font-medium hover:bg-hover_green_custom text-white">
           <a href={`${BASE_URL1}/api/v1/apply-job/get-cv-file?filename=${item.documentName}`} download>Download</a>
        </button>
        </div>
      </div>
      {/* </div> */}
