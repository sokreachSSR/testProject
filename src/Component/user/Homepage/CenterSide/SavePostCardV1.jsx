...
import { BASE_URL, BASE_URL1, BASE_URL3 } from "../../../../utils/Utils";

export default function SavePostCardV1({ item, index }) {
  const [like, setLike] = useState(false);
  const [save, setSave] = useState(false);
  const userDetail = useSelector((state) => state.userDetail.userDetail);

  const handleSave = () => {
    DeleteSavePost(item.savePostId).then(() => console.log(item.savePostId));
    setSave(!save);
  };

  const handleUnSave = () => {
    AddSavePost(item.post.postId).then(() => console.log(item.post.postId));
    setSave(!save);
  };
  ...
  // Save Post
  const [getSavePost, setSavePost] = useState([]);
  useEffect(() => {
    SavePost().then((e) => {
      setSavePost(e.data.payload);
    });
  }, []);
  ...
  return (
    <div>
      <div className="lg:[90%]">
        <div className="bg-white w-full h-fit rounded-[20px] shadow mb-3 pb-10">
          <div className="flex justify-between items-center">
            <a href="" className="flex ml-[30px] mt-[30px]">
              {item.post.company == true ? (
                <img
                  src={BASE_URL3 + item.post?.user?.companyProfile}
                  alt=""
                  className="w-[50px] h-[50px] rounded-full"
                />
              ) : (
                <img
                  src={BASE_URL3 + item.post?.user?.user_profile}
                  alt=""
                  className="w-[50px] h-[50px] rounded-full"
                />
              )}
              <div className="ml-3">
                {item.post.company == true ? (
                  <h3 className=" font-bold font-SecondaryFont text-[18px]">
                    {item.post?.user?.companyName}
                  </h3>
                ) : (
                  <h3 className=" font-bold font-SecondaryFont text-[18px]">
                ...
                <div className="flex items-center">
                  <span className="text-gray-400 pr-2">
                    {getTimeDifference(item.post?.time)}
                  </span>
                  <svg
                    width="13"
                    height="13"
            ...
            <div>
              ...
              {item.post?.postImage != null ? (
                <div className="flex justify-center px-[30px]">
                  <img
                    src={
                      BASE_URL + `/images/POST?fileName=` + item.post?.postImage
                    }
                    alt=""
          ...
          <div className="flex justify-between mx-[30px] mt-3">
            <div>
              <span>{item.post?.postInteract.Like_count} </span>
              <span>likes</span>
            </div>
            <div>
              <span>{item.post.postInteract.Comment_count} </span>
              <span>comment</span>
            </div>
          </div>
          {/* <div className="flex justify-around mx-[30px] border-b-2 pb-1">
            ...
            >
              ...
              <span className={like ? "text-[#DE5753]" : "text-black"}>
                Like
              </span>
            </div>
            <div className="flex items-center">
